...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        for i, row in tqdm(df.iterrows(), total=df.shape[0], desc=f'Loading {file_type}'):
            self.examples.append(convert_examples_to_features(row.func, row.target, tokenizer, args, idx=row.idx))

        # for i in tqdm(range(len(funcs))):
        #     self.examples.append(convert_examples_to_features(funcs[i], labels[i], tokenizer, args, idx=i))
        if file_type == "train":
...
def convert_examples_to_features(func, label, tokenizer, args, idx):
    ...
    return InputFeatures(source_tokens, source_ids, label, idx=idx)
...
def train(args, train_dataset, model, tokenizer, eval_dataset, monitor='eval_accuracy'):
    """ Train the model """
    # build dataloader
        ...
        for step, batch in enumerate(bar):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results[monitor] > best_metric:
                        best_metric = results[monitor]
                        logger.info("  "+"*"*20)  
                        logger.info("  Best %s:%s", monitor, round(best_metric,4))
                        logger.info("  "+"*"*20)                          
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    result = {
        "eval_accuracy": accuracy_score(y_trues, y_preds),
        "eval_recall": recall_score(y_trues, y_preds),
        "eval_precision": precision_score(y_trues, y_preds),
        "eval_f1": f1_score(y_trues, y_preds),
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    if args.predictions:
        pred_output = args.predictions
    else:
        pred_output = os.path.join(args.output_dir, 'predictions.txt')

    with open(pred_output,'w') as f:
        for example,pred in zip(test_dataset.examples, logits[:, 1]):
            f.write(f'{example.idx}\t{pred:.4f}\n')
    return
...
def main():
    ...
    parser.add_argument('--neg_weight', type=float, default=None)
    ...
    if args.neg_weight is None:
        if 'all' in args.train_data_file:
            args.neg_weight = 1.081
            args.pos_weight = 13.340
        elif 'draper' in args.train_data_file:
            args.neg_weight = 1.069
            args.pos_weight = 15.469
        else:
            args.neg_weight = 1
            args.pos_weight = 1
    ...
    class_weights = torch.tensor([args.neg_weight, args.pos_weight]).to(torch.float)
    print('Class Weights:', class_weights)
    model = Model(model, config, tokenizer, args, class_weights)
    print_params(model)

    logger.info("Training/evaluation parameters %s", args)
    # Training
    if args.do_train:
        if args.neg_weight == args.pos_weight:
        ...
        train(args, train_dataset, model, tokenizer, eval_dataset, monitor=monitor)

    # Evaluation
    results = {}
    if args.do_test:
        output_dir = os.path.join(args.output_dir, args.model_name)
        missing, unexpected = model.load_state_dict(torch.load(output_dir), strict=False)#, map_location=args.device))
        print('-- Missing Keys --')
        print(missing)
