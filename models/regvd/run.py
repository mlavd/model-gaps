...
def train(args, train_dataset, model, tokenizer, monitor='eval_accuracy'):
    """ Train the model """ 
...
def evaluate(args, model, tokenizer,eval_when_training=False):
    ...
    best_threshold = 0.5
    best_f1 = 0
    y_preds = logits[:,0]>best_threshold

    result = {
        "eval_accuracy": accuracy_score(labels, y_preds),
        "eval_recall": recall_score(labels, y_preds),
        "eval_precision": precision_score(labels, y_preds),
        "eval_f1": f1_score(labels, y_preds),
...
def test(args, model, tokenizer):
    ...
    with open(pred_output,'w') as f:
        for example,pred in zip(eval_dataset.examples,logits[:,0]):
...
def main():
    ...
    if args.model == "devign":
        ...
        class_weights = torch.tensor([args.neg_weight, args.pos_weight]).to(torch.float32).to('cuda')
        model = GNNReGVD(model, config, tokenizer, args, class_weights)
    ...
    if args.do_train:
        ...
        train(args, train_dataset, model, tokenizer, monitor=monitor)
